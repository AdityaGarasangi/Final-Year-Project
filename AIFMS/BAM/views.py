...
from .forms import (
    NewAccountsForm,
    UpdateAccountForm,
    AddNewInterAccountTransactionRecord,
)
...
def add_new_inter_account_transaction_record_view(request):
    ...
    if form.is_valid():
        ...
        from_account.account_balance = from_account.account_balance - float(
            form.data["amount"]
        )
        to_account.account_balance = to_account.account_balance + float(
            form.data["amount"]
        )
